using Gadfly
function getError(n_t::Integer, n_x::Integer)
  α = 4
  β = 2
  ...
  y_sol = y[n_t,:]
  y_ex = map((x) -> exact_solution((n_t-1)*τ, x), linspace(0, 1, n_x))

  return sqrt(sum((y_sol-y_ex).^2))
end

# xs = 2:100
# p = plot(x = xs, y = map(getError, xs), Theme(
#     panel_fill = colorant"white",
#   ),
#   Scale.y_log10,
#   Guide.xticks(ticks = collect(0:5:100)),
#   Guide.yticks(ticks = collect(0:5:100)),
#   Guide.title("L_2 error"),
#   Guide.xlabel("# Time steps"),
#   Guide.ylabel("L_2 error"))
#
# draw(PDF("l2error.pdf", 8inch, 6inch), p)
xs = 2:100
p = plot(x = xs, y = map((n) -> getError(n, 100), xs), Theme(
    panel_fill = colorant"white",
  ),
    Scale.y_log10,
    Guide.xticks(ticks = collect(0:5:100)),
    Guide.yticks(ticks = collect(0:5:100)),
    Guide.title("L_2 error, Pe = 400"),
    Guide.xlabel("# Time steps"),
    Guide.ylabel("L_2 error")
 )

draw(PDF("l2error_pe400_h_1_over_100.pdf", 8inch, 6inch), p)
